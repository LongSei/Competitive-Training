...
// MAIN CODE //
int getValueWithIndexInLevel(int index, int level) {
    int preIndex = (1 << (level - 1)) - 1;
    return preIndex + index;
}

ii getIndexAndLevel(int value) {
    int ll = 1, rr = 55; 
    int level = 0;
    while (ll <= rr) {
        int mid = (ll + rr) >> 1;
        if (getValueWithIndexInLevel(1, mid) <= value) {
            level = mid;
            ll = mid + 1;
        }
        else {
    ...
    return ii(level, value - getValueWithIndexInLevel(1, level) + 1);
}

int getParent(int value) {
    ii inf = getIndexAndLevel(value);
    int index = inf.second;
    int level = inf.first;
    int parentIndex = (index + 1) / 2;
    int parentLevel = level - 1;
    return getValueWithIndexInLevel(parentIndex, parentLevel);
}
