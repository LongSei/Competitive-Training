...
int n, x; 
int arr[MAX];
vector<int> listEffect;
ii  remainElement = ii(INF, -1);
void reset() {
	remainElement = ii(INF, -1);
	listEffect.clear();
}
void solve() {
	reset();
	cin >> n >> x;
	int amountEffectPrefix = n - x;
	int amountEffectSuffix = x + 1;
	for (int i = 1; i <= n; i++) {
		int t; cin >> t;
		arr[i] = t;
		
		if (i <= amountEffectPrefix || i >= amountEffectSuffix) {
			listEffect.pb(arr[i]);
		}
		else {
			remainElement = ii(min(remainElement.first, arr[i]), max(remainElement.second, arr[i]));
		}
	}
	sort(all(listEffect));
	if (amountEffectPrefix + amountEffectPrefix >= n) {
		cout << "YES" << endl;
		return;
	}
	if (listEffect.empty() == true) {
		bool isok = true;
		for (int i = 1; i <= n - 1; i++) {
			if (arr[i] > arr[i + 1]) {
				isok = false;
	...
	// debug(amountEffectPrefix, amountEffectSuffix, listEffect);
	if (remainElement.first >= listEffect[amountEffectPrefix - 1] && remainElement.second <= listEffect[amountEffectPrefix]) {
		cout << "YES" << endl;
		return;
	}
	cout << "NO" << endl;
